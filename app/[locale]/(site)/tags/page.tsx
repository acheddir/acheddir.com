...
export async function generateMetadata(props: TagProps): Promise<Metadata> {
  ...
  const title = t(siteMetadata.title.default);

  return {
    title: t('tags.title'),
    description: t('tags.allin', { title: title })
  };
}

export default async function TagsPage(props: TagProps) {
  const { locale } = await props.params;
  ...
  return (
    <div className="container mb-4">
      <div className="prose mx-auto max-w-5xl dark:prose-invert prose-headings:font-heading prose-headings:font-bold prose-headings:leading-tight hover:prose-a:text-accent-foreground prose-a:prose-headings:no-underline">
        <h1 className="mt-0">{t("tags.all")}</h1>
        <hr className="my-4" />
